 ...
 */
class SubmissionSearch extends Paperwork
{
    /**
     * @inheritdoc
     */
    ...
    {
        $pos_id = Yii::$app->user->identity->position_id;

        if ($pos_id == 28) {
            $query = Paperwork::find()
                ->where(['status_by_su'=> 0]);
        }
        if ($pos_id == 27) {
            $query = Paperwork::find()
                ->where(['status_by_bendahari'=> 0]);
        }
        if ($pos_id == 26) {
            $query = Paperwork::find()
                ->where(['status_by_timbalan'=> 0]);
        }
        if ($pos_id == 25) {
            $query = Paperwork::find()
                ->where(['status_by_nazir'=> 0]);
        }
